					...
					for APP_SCOPE in ${APP_SCOPES[@]};do
						...
						for Pa in ${Pas[@]};do
							...
							for Pb in ${Pbs[@]};do
								...
								while [[ $i -lt $RUNS ]];do
									$SCRIPTS/genRule-Trace-single.sh $PARAM_FILE $NUM_RULES $SMOOTH $ADDRESS_SCOPE $APP_SCOPE $Pa $Pb $SCALE $i> /dev/null 2> /dev/null
								 	# gdb --args $CUTSPLIT -r $SCRIPTS/rules/$PARAM_FILE-$NUM_RULES-$SMOOTH-$ADDRESS_SCOPE-$APP_SCOPE-$i.rules -e $SCRIPTS/traces/$PARAM_FILE-$NUM_RULES-$SMOOTH-$ADDRESS_SCOPE-$APP_SCOPE-$Pa-$Pb-$SCALE-$i.trace -c 1 -u 0
								 	# (taskset --cpu-list $i $CUTSPLIT -r $SCRIPTS/rules/$PARAM_FILE-$NUM_RULES-$SMOOTH-$ADDRESS_SCOPE-$APP_SCOPE-$i.rules -e $SCRIPTS/traces/$PARAM_FILE-$NUM_RULES-$SMOOTH-$ADDRESS_SCOPE-$APP_SCOPE-$Pa-$Pb-$SCALE-$i.trace -c 1 -u 0 > /tmp/output_$i; nodes=$(cat /tmp/output_$i | grep "AverageTraversed" | awk '{print $2}'); mem=$(cat /tmp/output_$i | grep "TotalMemoryBytes" | awk '{print $2}'); echo $mem >> /tmp/output-mems; echo $nodes >> /tmp/output-runs) &
