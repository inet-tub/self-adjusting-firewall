...
if n < 50:
    ...
    m = 50
    samples = random.sample(G.nodes(), k = m)
else:
    m = int(n*0.1)
    samples = random.sample(G.nodes(), k = m)
...
avg_ancestors = 0.0
for i in samples:
    avg_ancestors += len(nx.ancestors(G, i))
avg_ancestors /= float(m)
print("avg_ancestors: {:.2f}".format(avg_ancestors))


with open(outputfile, "w") as file:
    file.write(f"{ruleset};{prio};{nest_value};{len(depth)};{avg_degree:.2f};{avg_ancestors:.2f};\n")
